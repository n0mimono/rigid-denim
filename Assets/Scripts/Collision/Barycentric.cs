// ref: http://wiki.unity3d.com/index.php/Barycentric
...
public struct Barycentric
{
  ...
  public Vector3 InterpolateInside(Vector3 v1, Vector3 v2, Vector3 v3)
  {
    Vector3 q = Interpolate(v1, v2, v3);

    if (u < 0f) {
      return PutInside (v2, v3, q);
    } else if (v < 0f) {
      return PutInside (v3, v1, q);
    } else if (w < 0f) {
      return PutInside (v1, v2, q);
    } else {
      return q;
    }
  ...
  private Vector3 PutInside(Vector3 v1, Vector3 v2, Vector3 q) {
    ...
    float c = Vector3.Dot (p, u);
    float a = c / r.magnitude;
    if (a < 0f) {
      return v1;
    } else if (a > 1f) {
      return v2;
